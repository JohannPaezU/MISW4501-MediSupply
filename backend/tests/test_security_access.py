...
class TestSecurityAccess(BaseTest):
    ...
    invalid_user_token: str = create_access_token(data={"sub": "nonexistent", "role": "invalid"})
    invalid_data_token: str = create_access_token(data={"no-sub": "nonexistent", "no-role": "invalid"})
    ...
    def test_access_denied_with_token_causing_exception(self, mock_jwt_decode):
        ...
        assert response.json()["message"] == "An error occurred while processing the token."
    ...
    def test_access_denied_for_institutional_user(self):
        ...
        assert response.json()["message"] == "Access denied: requires one of the following roles: admin"

    def test_access_denied_for_commercial_user(self):
        response = self.client.get(
            self.endpoint,
            headers={"Authorization": f"Bearer {self.commercial_token}"}
        )
        assert response.status_code == 403
        assert response.json()["message"] == "Access denied: requires one of the following roles: admin"
