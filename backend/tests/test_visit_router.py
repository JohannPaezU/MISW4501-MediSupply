...
class TestVisitRouter(BaseTest):
    ...
    @pytest.mark.parametrize("authorized_client", ["institutional_token"], indirect=True)
    ...
    def test_register_visit_success(self, mock_get_validated_address, authorized_client):
        payload = self.create_visit_payload.copy()
    ...
    def test_get_all_visits_with_expected_date_filter(self, authorized_client):
        expected_date = date.today().replace(day=date.today().day + 1).isoformat()
        response = authorized_client.get(f"{self.prefix}/visits?expected_date={expected_date}")
        json_response = response.json()
    ...
    def test_get_all_visits_with_status_filter(self, authorized_client):
        ...
        response = authorized_client.get(f"{self.prefix}/visits?visit_status={visit_status}")
        json_response = response.json()
    ...
    def test_report_visit_with_early_visit_date(self, authorized_client):
        visit = next(v for v in self.visits if v.status == VisitStatus.PENDING)
        data = self.report_visit_data.copy()
        data["visit_date"] = visit.expected_date.replace(day=visit.expected_date.day - 1).isoformat()
    ...
    @pytest.mark.parametrize("authorized_client", ["commercial_token"], indirect=True)
    @patch("src.services.visit_service._validate_visual_evidence", side_effect=Exception("Unexpected error"))
    ...
    @pytest.mark.parametrize("authorized_client", ["commercial_token"], indirect=True)
    def test_report_visit_with_invalid_visual_evidence_format(
    ...
    @pytest.mark.parametrize("authorized_client", ["commercial_token"], indirect=True)
    def test_report_visit_with_invalid_visual_evidence_size(
            self, authorized_client
    ):
        visit = next(v for v in self.visits if v.status == VisitStatus.PENDING)
        data = self.report_visit_data.copy()
        ...
        json_response = response.json()
        assert response.status_code == 400
        assert json_response["message"] == "Visual evidence file size exceeds the maximum limit of 30 MB"
    ...
    @pytest.mark.parametrize("authorized_client", ["commercial_token"], indirect=True)
    def test_report_visit_success_with_visual_evidence(
        self, authorized_client
    ):
        visit = next(v for v in self.visits if v.status == VisitStatus.PENDING)
        data = self.report_visit_data.copy()
