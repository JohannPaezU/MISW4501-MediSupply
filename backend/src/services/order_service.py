...
from src.errors.errors import ConflictException, NotFoundException, ApiError, BadRequestException
from src.models.db_models import Order, User, OrderProduct
...
def create_order(*, db: Session, order_create_request: OrderCreateRequest, client_id: str, seller_id: str | None = None) -> Order:
    _validate_order_request(db=db, order_create_request=order_create_request, client_id=client_id, seller_id=seller_id)
    order = Order(
...
def _validate_order_request(*, db: Session, order_create_request: OrderCreateRequest, client_id: str, seller_id: str | None = None) -> None:
    ...
    if duplicated_ids:
        raise BadRequestException(f"Duplicated product IDs in order: {', '.join(duplicated_ids)}")

    distribution_center = get_distribution_center_by_id(db=db, distribution_center_id=order_create_request.distribution_center_id)
    client = get_institutional_client_for_seller(db=db, seller_id=seller_id, client_id=client_id) if seller_id else get_user_by_id(db=db, user_id=client_id)
